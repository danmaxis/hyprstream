...
/**
 * Build a child-process env that overrides HIS+XDG_RUNTIME_DIR with the
 * resolved values. Used only if anyone ever needs to spawn a child that
 * speaks to Hyprland — the plugin itself talks to the socket directly now.
 */
export function hyprChildEnv(
  resolved: ResolvedHyprEnv,
  base: NodeJS.ProcessEnv = process.env,
): NodeJS.ProcessEnv {
  const out: NodeJS.ProcessEnv = { ...base };
  out.XDG_RUNTIME_DIR = resolved.runtimeDir;
  if (resolved.instanceSignature) {
    out.HYPRLAND_INSTANCE_SIGNATURE = resolved.instanceSignature;
  }
  return out;
}
